...
class Residual(torch.nn.Module):
    ...
    def __init__(self, in_ch, out_ch, s, r, fused=True):
        ...
        if fused:
            ...
            features = [Conv(in_ch, r * in_ch, torch.nn.SiLU()),
...
class EfficientNet(torch.nn.Module):
    def __init__(self, num_class=1000) -> None:
        ...
        for i in range(15):
            if i == 0:
                feature.append(Residual(filters[4], filters[5], 2, 6, gate_fn[1]))
            else:
                feature.append(Residual(filters[5], filters[5], 1, 6, gate_fn[1]))
        feature.append(Conv(filters[5], filters[6], torch.nn.SiLU()))
